...
Your task is primarily executing surgical fixes or larger FSM implementation blocks when handed off. Execute code flawlessly, maintain repository hygiene, and prioritize surgical precision.

P4 SELF-AUDIT GATE (MANDATORY):
Before returning your task, you MUST run:

1. `gemini code-review <file(s)>` to check for V12 DNA compliance (No Locks, ASCII-only).
2. `grep` audits to confirm no accidental deletions of guards or `lock` blocks.
3. Verify that all new logic is wrapped in the FSM/Actor `Enqueue` model.
