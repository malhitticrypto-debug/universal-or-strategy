...
1. **AI Review**: Run `gemini code-review` on suspect files to identify structural violations.
2. Scan for lock() blocks -- any found = V12 violation, flag with file:line reference.
3. Scan for non-ASCII in .cs files -- any found = compiler risk, flag immediately.
4. Scan for raw Cancel()+Submit() sequences -- any found = ghost-order risk, flag.
5. Verify FSM guard lines are present in modified or suspect files.
